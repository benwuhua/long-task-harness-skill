...
- Read `docs/runbooks/*.md` and `artifacts/README.md` if they exist — these are part of the repo-local harness being validated
...
List the repo-local harness assets that claim to support the system:
- startup/reset/seed commands
- diagnosis/recovery runbooks
- diagnostic or smoke scripts
- artifact bundle locations
- mechanical enforcement checks for architectural boundaries

For each asset, define how ST will verify that a fresh engineer can use it without oral context.
...
- **Diagnosability / operability** (if required by SRS or implied by the design/runtime): seed a failure, then verify a fresh engineer can localize it within the required time using only repo-local docs, scripts, and saved artifacts. Missing or stale harness assets are defects.
...
Harness defects follow the same rule:
- If a runbook, command, artifact path, or mechanical check is stale enough to mislead the next agent, treat it as a defect
- If the stale harness blocks diagnosis or safe operation, classify it as Major
...
- Update or delete stale harness assets discovered during ST: `env-guide.md`, `long-task-guide.md`, `docs/runbooks/*.md`, `artifacts/README.md`, and any `scripts/diagnose-*` / `scripts/reset-*` / `scripts/smoke-*`
...
- **Repo-local harness is part of release quality** — stale or missing runbooks, commands, artifact contracts, or enforcement checks are defects, not documentation debt
...
**Reads:** `feature-list.json`, `docs/plans/*-srs.md`, `docs/plans/*-design.md`, `docs/plans/*-ucd.md` (if UI), `docs/test-cases/feature-*.md` (per-feature ST from `long-task-feature-st`), `task-progress.md`, `env-guide.md`, `docs/runbooks/*.md`, `artifacts/README.md`, project config file (if applicable)
