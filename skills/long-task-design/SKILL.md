...
3. Extract key design drivers:
   ...
   - **Diagnosability / operability needs** — startup, reset, seed, logging, tracing, artifact capture, and failure-localization expectations that require repo-local support
...
1. **Architecture** — system components, logical view, tech stack decisions
   ...
   - Must identify boundaries that should be mechanically enforced (for example import rules, contract tests, layer checks) when repeated violations would be costly
...
7. **Testing strategy** — test types, coverage targets, tooling
   ...
   - Must define the repo-local evaluation harness for risky workflows and NFRs: reproduce/reset commands, seed strategy, artifact capture, success markers, and any mechanical checks needed to keep the system legible to future agents
8. **Deployment / infrastructure** (if applicable) — hosting, CI/CD, environments
   - Must define runtime observability surfaces needed by workers and ST: logs, health checks, trace/correlation IDs, screenshot or capture paths, and where artifact bundles live
...
**Harness-first completeness rule:** if a requirement or NFR cannot be exercised, reproduced, or diagnosed from repo-local scripts/docs/artifacts, the design is incomplete. Do not defer that thinking to implementation.
...
1. Summarize key inputs the Initializer will need:
   ...
   - **From Harness design**: required runbooks, diagnose/reset/smoke commands, artifact locations, and mechanical enforcement checks
...
| "We'll figure out debugging and observability during implementation" | If the repo-local harness is not designed now, the NFR is not real yet. |
| "Review can catch architecture drift later" | Repeated drift should be converted into mechanical enforcement during design. |
