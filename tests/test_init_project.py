...
env-guide.md and init.sh/init.ps1 are NOT generated by init_project.py — they are created
...
def test_creates_all_artifacts():
    ...
    try:
        ...
        expected_files = [
            ...
            os.path.join("artifacts", "screenshots", "README.md"),
            os.path.join("artifacts", "traces", "README.md"),
            os.path.join("artifacts", "reproductions", "README.md"),
        ...
        expected_dirs = [
            "scripts",
            "artifacts",
            os.path.join("artifacts", "logs"),
            os.path.join("artifacts", "reports"),
            os.path.join("artifacts", "screenshots"),
            os.path.join("artifacts", "traces"),
            os.path.join("artifacts", "reproductions"),
            "examples",
...
def test_artifact_subdir_readmes_created():
    """artifacts subdirectories should exist with purpose-specific README placeholders."""
    tmp = tempfile.mkdtemp()
    try:
        run_init("test-project", tmp)
        expectations = {
            ...
            "traces": ("trace", "network"),
            "reproductions": ("repro", "input"),
        }
        for subdir, keywords in expectations.items():
            readme = os.path.join(tmp, "artifacts", subdir, "README.md")
            assert os.path.exists(readme), f"Missing README for artifacts/{subdir}"
            with open(readme, "r", encoding="utf-8") as f:
                content = f.read().lower()
            assert all(keyword in content for keyword in keywords), \
                f"README for artifacts/{subdir} should mention {keywords}"
    finally:
        shutil.rmtree(tmp)
...
def test_creates_claude_md():
    ...
    try:
        ...
        with open(cm_path, "r", encoding="utf-8") as f:
            content = f.read()
        assert "<!-- long-task-harness-skill -->" in content, "Should contain marker"
...
def test_idempotent_claude_md():
    ...
    try:
        ...
        with open(cm_path, "r", encoding="utf-8") as f:
            content = f.read()
        count = content.count("<!-- long-task-harness-skill -->")
...
if __name__ == "__main__":
    tests = [
        ...
        test_claude_md_contains_key_file_references,
